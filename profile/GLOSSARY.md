...
* **The tool that renders it: `hee render view`.** Named 2026-09-08 (Spencer, direct: *"render-review.py, Gold and render-preview -- all the same thing, view"*). `.github`'s `bin/render-review.py` is the reference implementation above; `hee render view` is the name that implementation carries once it lives as a real `hee` subcommand. **view.lab.tcos.us is built with `hee render view`, and `hee render view` is how any surface in this org gets a pretty page.** Three names for one thing -- "render-review", "render-preview", "Gold's reference implementation" -- collapse to it; this entry exists so the fourth name never gets coined.
  * `render` is a **family**, not a single command: `hee render view` for pages, plus subcommands for the other things this org already renders. Real candidates that exist today and would fold in rather than be rewritten -- `hee-exif` (image metadata), `hee-print` (*"render files nicely for terminal surfaces"*), `hee-qr`. Rule 15 applies in the usual direction: fold existing tools into the family, do not write new ones beside them.
  * **Recorded, not yet built.** Spencer, 2026-09-08, emphatically: *"I do NOT NOT NOT want to get off track building our infra."* This is a naming and boundary decision captured so it survives; the implementation is scheduled work, not in-flight work. `render-review.py` remains the live reference implementation until it moves.
