...
* **Universal components** (required on every real Gold page, enforced by `check_gold_og.py`): tokens (accent, both light/dark), the font pair, the theme toggle + pre-paint script, the font-size toggle (S/M/L/XL/XXL), the `lu:` freshness row, and the full OG set.
* **Advanced components** (real, available, content-dependent -- not required on every page, per Spencer direct, 2026-08-28: "includes pretty print and hover and exif and all the goodies"): pretty-print (`.github`'s `bin/render-review.py`, `get_pretty_html()`), hover-preview for linked files, and EXIF-signed-image display with an "EXIF signed" badge -- all three already real and live on `view.lab.tcos.us/contracts.html`, not yet ported anywhere else. Not checked by `check_gold_og.py` -- a simple list page (blog-hub.html) has no file/image content for these to apply to; use where the content actually calls for it.
